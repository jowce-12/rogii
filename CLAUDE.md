...
## 티어별 최적 배합 — 차등 측정 (2026-07-25, 편향이 상쇄되는 근거)

우물군별 pole 성능(정직 poles, 행 가중 pooled RMSE)과 분산-최적 배합(최소제곱):

| 우물군 | fleongg | GRU | sp45 | 최적배합 sp45/fl/GRU |
|---|---|---|---|---|
| 일반 (180우물) | 5.896 | 6.532 | 6.209 | 0.25 / 0.54 / **0.22** |
| 몬스터 (120우물) | 9.453 | 9.493 | 10.007 | **0.01** / 0.50 / **0.49** |

- **몬스터는 GRU를 2배 이상 받아야 한다**(0.49 vs 0.22). 기제: 일반 우물에서 fleongg가 GRU보다 0.64ft 앞서지만 몬스터에서는 격차가 0.04ft로 소멸 — PF 증거가 약한 우물에서 행 단위 GBM의 국소 신호가 부실해지고 시퀀스 모델이 상대적으로 살아남.
- **몬스터에서 sp45는 사실상 0**(0.01): 셋 중 가장 약하고(10.007) fleongg와의 상관도 가장 높다(0.892) → 탈상관 프리미엄도 없음. 일반 우물에서는 0.25를 받을 만함(상관 0.863, 성능 6.209).
- 절대 수준은 하네스 앙상블 편향으로 낮게 나오지만, **두 군의 차이(차등)는 편향이 상쇄되어 신뢰 가능**.
- → 몬스터 티어 방향: **sp45 ↓, GRU ↑** (일반 대비). 제출된 M2 `0.15/0.35/0.50`은 GRU는 맞고 sp45는 반대. 다음 프로브 권고: `python patch53.py 0.05 0.40 0.55`.
