#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Auto-generated from public-score-rogii-lb-7-159.ipynb
# ROGII - Wellbore Geology Prediction. All notebook cells concatenated in order.

# ===== CELL 0 [code] id=7a871d3e =====
import os
os.environ["ROGII_GOLD_PROFILE"] = "conservative"

# ===== CELL 1 [code] id=a0e02cdb =====
...
def apply_selector_variant(name, pf_by_scale, tvt_beam, last_known_tvt):
    ...
    # Optional CV knob: gently hedge the hand-tuned selector toward the simple
    # pf12/beam blend when local CV says the hard selector is overfitting.
    blend_to_pf12_beam = float(os.environ.get('ROGII_SELECTOR_PF12_BEAM_BLEND', '0.0'))
    if blend_to_pf12_beam > 0.0 and 'pf_scale_12' in pf_by_scale:
        alt = 0.8 * pf_by_scale['pf_scale_12'] + 0.2 * tvt_beam
        w = float(np.clip(blend_to_pf12_beam, 0.0, 1.0))
        pred = (1.0 - w) * pred + w * alt
    return pred

# ===== CELL 5 [code] id=augtw0001 =====
# === Self-augmented typewell (PPT slide-9: horizontal pre-PS GR has higher
...
def run_pf_ancc(hw,tw_tvt,tw_gr,N=ANCC_N):
    ...
    gr_fill=hw['GR'].astype(float).interpolate(limit_direction='both').fillna(float(np.nanmean(tw_gr)))
    pts,std=_pf_ancc(ev['MD'].values.astype(np.float64),ev['Z'].values.astype(np.float64),
                      gr_fill.loc[ev.index].values.astype(np.float64),gg,gmin,gst,
...
def run_pf_z(hw,tw_tvt,tw_gr,N=PF_N):
    ...
    pts,std=_pf_z(ev['MD'].values.astype(np.float64),ev['Z'].values.astype(np.float64),
                   gr_fill.loc[ev.index].values.astype(np.float64),
...
# ===== CELL 12 [code] id=e22a8309 =====
oof_preds = pd.DataFrame(oof_preds)
test_preds = pd.DataFrame(test_preds)

# ===== CELL 13 [code] id=c256435a =====
...
for i, wid in enumerate(test_wells):
    ...
    try:
        ...
        pf_by_scale_aug = run_pf_lik_ensemble_scales(hw_te, tw_aug, n_particles=400, n_seeds=48)  # cheap: ~30%% of base PF cost, keeps the CV gain (9h budget)
...
# ===== CELL 19 [code] id=7941a648 =====
sub_2 = pd.DataFrame(rows)

# ===== CELL 20 [code] id=7376c2f6 =====
sub = (
...
_GOLD_INCLUDE_AUG_PF = _gold_os.environ.get('ROGII_GOLD_INCLUDE_AUG_PF', '0') == '1'
    ...
    try:
        ...
        for variant in variants:
            try:
                ...
                pass
        if _GOLD_INCLUDE_AUG_PF and callable(globals().get('build_aug_typewell')):
            try:
                tw_aug = build_aug_typewell(hw_masked, tw)
                pf_by_scale_aug = run_pf_lik_ensemble_scales(
                    hw_masked,
                    tw_aug,
                    scales=tuple(globals().get('SELECTOR_SCALES', (3.0, 5.0, 8.0, 12.0))),
                    n_particles=max(64, int(0.7 * int(n_particles))),
                    n_seeds=max(8, int(n_seeds) // 2),
                )
                for variant in variants:
                    try:
                        pred = apply_selector_variant(variant, pf_by_scale_aug, tvt_beam, last_known_tvt)
                        if pred is not None and len(pred) == len(hw_masked):
                            out['pf_aug|' + variant] = _gold_np.asarray(pred, dtype=float)
                    except Exception:
                        pass
            except Exception as e:
                print('Augmented PF calibration skipped:', e)
    except Exception as e:
...
if not _GOLD_ENABLE:
    ...
    print('Gold visible-prefix calibration:', dict(
        ...
        include_pf=_GOLD_INCLUDE_PF,
        include_aug_pf=_GOLD_INCLUDE_AUG_PF,
    ...
    for _wi, _wid in enumerate(_gold_wells, 1):
        try:
            ...
            if _rep.get('status') == 'ok':
                ...
                _need_pf_final = str(_best_name).startswith('pf|') or str(_best_name).startswith('pf_aug|')
