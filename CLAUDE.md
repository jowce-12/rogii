...
**현재 노트북 = LB 최고 구성 + 다음 프로브**: 일반 티어 `0.10/0.40/0.50`(patch52, LB 6.590 확인) + 몬스터 티어 `0.10/0.35/0.55`(patch53, 신규). patch53 사유: 사다리를 올리면서 몬스터(전체 행의 41%)가 일반 우물보다 GRU를 **덜** 받는 역전이 생겼음 — patch49의 원래 취지(몬스터는 GRU를 더)에 맞게 복원.
