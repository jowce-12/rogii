# A/B proxy for cross-tracker disagreement features. DEPLOYED-EQUIVALENT BASE: cache
# features + the 4 STRIDE join cols (as train_stack uses). B adds the 8 xtrk cols.
# Same harness family as measure_mmprof (150w fixed subset, midreg LGB, GKF-4, same folds).
...
xt = pd.read_parquet("xtrk_join.parquet")
XT_COLS = [c for c in xt.columns if c != "id"]
df = df.merge(xt, on="id", how="left")
print(f"[{time.time()-t0:.0f}s] {len(df)} rows, xtrk coverage {df[XT_COLS[0]].notna().mean():.1%}", flush=True)

base_cols = [c for c in df.columns
             if c not in ("id", "well", "target") and c not in XT_COLS
...
rB, impB = run(base_cols + XT_COLS, "B +xtrk")
print(f"\nDELTA (B-A) = {rB - rA:+.4f}  ({'xtrk helps' if rB < rA else 'no gain'})", flush=True)
