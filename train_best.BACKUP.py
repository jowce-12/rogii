...
def _feat_cache_name(_n):
    return f"train_features_v5_f{os.environ.get('ROGII_FEATS','1')}_w{_n}.parquet"
...
for ci, params in enumerate(lgb_cfgs):
    ...
    print(f"{name} OOF RMSE={rmse(y, oof):.4f}  (avg best_iter={full_iters[name]})", flush=True)
...
if use_cb:
    for ci, params in enumerate(cb_cfgs):
        ...
        for tr, va in cv.split(X, y, groups=g):
            ...
            m.fit(X[tr], y[tr], eval_set=[(X[va], y[va])], use_best_model=True, early_stopping_rounds=250)
...
for tr, va in cv.split(OOF, y, groups=g):
...
for ci, params in enumerate(lgb_cfgs):
    ...
    m = LGBMRegressor(**p); m.fit(X, y); joblib.dump(m, outdir / f"lgb{ci}.pkl")
    print(f"saved lgb{ci}.pkl (n_estimators={p['n_estimators']})", flush=True)
if use_cb:
    for ci, params in enumerate(cb_cfgs):
        p = dict(params); p["iterations"] = full_iters[f"cb{ci}"]
        m = CatBoostRegressor(**p); m.fit(X, y); m.save_model(str(outdir / f"cb{ci}.cbm"))
        print(f"saved cb{ci}.cbm (iterations={p['iterations']})", flush=True)
joblib.dump(ridge, outdir / "ridge.pkl")
json.dump({"base_names": base_names, "use_cb": use_cb, "ridge_oof_rmse": float(rmse(y, meta_oof))},
