...
for lam in LAMS:
    ...
    if r < best[0]:
        best = (r, lam, rows)
pd.concat(best[2], ignore_index=True).to_parquet("gru_oof_dipfused.parquet", index=False)
print(f"BEST lam={best[1]} -> gru_oof_dipfused.parquet ({best[0]:.4f})", flush=True)
