...
LAMS = (1024.0, 2048.0, 4096.0, 8192.0)
TAGS = ("_da", "_db", "_dc")
...
for lam in LAMS:
    ...
    if r < best[0]:
        best = (r, lam, rows)
pd.concat(best[2], ignore_index=True).to_parquet("gru_oof_dipfused_ext.parquet", index=False)
print(f"BEST lam={best[1]} -> gru_oof_dipfused_ext.parquet ({best[0]:.4f})", flush=True)
